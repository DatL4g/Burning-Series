...
data object BSUtil {
    ...
    fun commonSeriesHref(href: String): String {
        return SeriesData.fromHref(fixSeriesHref(href)).copy(
            season = null,
            language = null
        ).toHref()
    }
    ...
    data class SeriesData(
        ...
        fun toSTO(): String {
            return if (season != null) {
                val seasonSource = if (season <= 0) {
                    "filme"
                } else {
                    "staffel-$season"
                }

                "serie/stream/${source.lowercase()}/$seasonSource"
            } else {
                "serie/stream/${source.lowercase()}"
            }
        }

        companion object {
            fun fromHref(href: String): SeriesData {
                fun getTitle(): String {
                ...
                return SeriesData(
                    source = title,
                    season = season?.ifBlank { null }?.toIntOrNull(),
